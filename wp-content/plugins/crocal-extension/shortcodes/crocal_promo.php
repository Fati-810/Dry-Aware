...
if( !function_exists( 'crocal_ext_vce_promo_shortcode' ) ) {

	function crocal_ext_vce_promo_shortcode( $atts, $content ) {
		...
		if ( !empty( $image ) ) {
			...
			$image_classes[] = 'eut-expandable-info-logo';
			...
			if ( !empty( $retina_image ) ) {
				...
				$image_string = crocal_ext_vce_get_attachment_image( $img_id, 'full' , "", array( 'class' => $image_class_string, 'srcset'=> $image_srcset ) );
			} else {
				$image_string = crocal_ext_vce_get_attachment_image( $img_id, 'full' , "", array( 'class' => $image_class_string ) );
			}

		}

		$output .= '<div class="eut-element eut-expandable-info eut-align-' . esc_attr( $align ) . ' ' . esc_attr( $el_class ) . '">';
		$output .= $image_string;
		$output .= '  <div class="eut-expandable-info-content">';
		$output .= '  	<div class="eut-expandable-info-space"></div>';
		$output .= '    <p class="eut-leader-text">' . crocal_ext_vce_unautop( $content ) . '</p>';
		$output .= $button;
		$output .= '  </div>';
		$output .= '</div>';
	...
	add_shortcode( 'crocal_promo', 'crocal_ext_vce_promo_shortcode' );
...
if( !function_exists( 'crocal_ext_vce_promo_shortcode_params' ) ) {
	function crocal_ext_vce_promo_shortcode_params( $tag ) {

		$crocal_ext_vce_promo_shortcode_btn_params = crocal_ext_vce_get_button_params();
		$crocal_ext_vce_promo_shortcode_params = array_merge(
			array(
				array(
					"type" => "attach_image",
			...
			$crocal_ext_vce_promo_shortcode_btn_params
		);

		return array(
			"name" => esc_html__( "Advanced Promo", "crocal-extension" ),
			"description" => esc_html__( "Advanced, impressive promotion for whatever you like", "crocal-extension" ),
			"base" => $tag,
			"class" => "",
			"icon"      => "icon-wpb-eut-promo",
			"category" => esc_html__( "Content", "js_composer" ),
			"params" => $crocal_ext_vce_promo_shortcode_params,
		);
	}
}

if( function_exists( 'vc_lean_map' ) ) {
	vc_lean_map( 'crocal_promo', 'crocal_ext_vce_promo_shortcode_params' );
} else if( function_exists( 'vc_map' ) ) {
	$attributes = crocal_ext_vce_promo_shortcode_params( 'crocal_promo' );
	vc_map( $attributes );
}
