<?php
/**
 * Euthemians Latest Comments
 * A widget that displays latest comments.
 * @author		Euthemians Team
 * @URI			http://euthemians.com
 */

class Crocal_Ext_Widget_Latest_Comments extends WP_Widget {

	function __construct() {
		$widget_ops = array(
			'classname' => 'eut-comments',
			'description' => esc_html__( 'A widget that displays latest comments', 'crocal-extension' ),
		);
		$control_ops = array(
			'width' => 300,
			'height' => 400,
		...
		parent::__construct( 'eut-widget-latest-comments', '(Euthemians) ' . esc_html__('Latest Comments', 'crocal-extension' ), $widget_ops, $control_ops );
	}

	function crocal_ext_widget_latest_comments() {
		$this->__construct();
	}
	...
	function widget( $args, $instance ) {
				...
				<li>
					...
					<div class="eut-comment-content">
						<div class="eut-author">
							<?php echo sprintf( _x('%1$s on %2$s', 'Author *on* Post Title', 'crocal-extension'), get_comment_author_link( $comment->comment_ID ), '<a href="' . esc_url( get_comment_link( $comment->comment_ID ) ) . '">' . get_the_title( $comment->comment_post_ID ) . '</a>'); ?>
						</div>
						<div class="eut-comment-date"><?php echo esc_html( get_comment_date( '', $comment->comment_ID ) ); ?></div>
		...
		<?php
			...
			echo esc_html__( 'No Comments Found!', 'crocal-extension' );
		}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'num_of_comments' ) ); ?>"><?php echo esc_html__( 'Number of comments:', 'crocal-extension' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_avatar' ) ); ?>"><?php echo esc_html__( 'Show Avatar:', 'crocal-extension' ); ?></label>
