 ...
 * @version   2.6.1 for parent theme Crocal for publication on ThemeForest
...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			// Load class strings.
			$this->strings = array(
				'page_title'                      => __( 'Theme Plugins', 'crocal' ),
				'menu_title'                      => __( 'Theme Plugins', 'crocal' ),
				...
				'return'                          => __( 'Return to Theme Plugins Installer', 'crocal' ),
		...
		public function install_plugins_page() {
			...
			<div class="tgmpa wrap">
				...
				<?php do_action( 'crocal_eutf_before_tgmpa_plugins' ); ?>
			...
			foreach ( $plugins[ $this->view_context ] as $slug => $plugin ) {
				...
				if ( $plugin['required'] ) {
					if ( ! $this->tgmpa->is_plugin_installed( $slug ) ) {
						$table_data[ $i ]['upgrade_notice'] = $plugin['name'] . ' ' . esc_html__( 'is a required plugin and needs to be installed.', 'crocal' );
						add_action( "tgmpa_after_plugin_row_{$slug}", array( $this, 'crocal_eutf_plugin_update_row' ), 10, 2 );
					} elseif( $this->tgmpa->does_plugin_require_update( $slug ) ) {
						$table_data[ $i ]['upgrade_notice'] = $plugin['name'] . ' ' . esc_html__( 'is a required plugin and needs to be updated.', 'crocal' );
						add_action( "tgmpa_after_plugin_row_{$slug}", array( $this, 'crocal_eutf_plugin_update_row' ), 10, 2 );
					}else if( ! $this->tgmpa->is_plugin_active( $slug ) ) {
						$table_data[ $i ]['upgrade_notice'] = $plugin['name'] . ' ' . esc_html__( 'is a required plugin and needs to be activated.', 'crocal' );
						add_action( "tgmpa_after_plugin_row_{$slug}", array( $this, 'crocal_eutf_plugin_update_row' ), 10, 2 );
					}
				}
				...
				<tr class="plugin-update-tr">
					<td colspan="', absint( $this->get_column_count() ), '" class="plugin-update colspanchange">
						<div class="update-message notice inline notice-warning notice-alt"><p>',
							...
							' <strong>', wp_kses_data( $item['upgrade_notice'] ), '</strong></p>
						</div>
					</td>
				</tr>';
		}
		public function crocal_eutf_plugin_update_row( $slug, $item ) {

			echo '
				<tr class="plugin-update-tr">
					<td colspan="', absint( $this->get_column_count() ), '" class="plugin-update colspanchange">
						<div class="update-message notice inline notice-warning notice-alt">',
							'<p><strong>', wp_kses_data( $item['upgrade_notice'] ), '</strong></p>
						</div>
					</td>
				</tr>';
		}
