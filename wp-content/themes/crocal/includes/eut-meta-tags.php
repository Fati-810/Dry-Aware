...
if ( !function_exists('crocal_eutf_opengraph_add_prefix') ) {
	function crocal_eutf_opengraph_add_prefix( $output ) {
		if ( crocal_eutf_visibility( 'meta_opengraph' ) ) {
			$prefixes = array(
				'og' => 'http://ogp.me/ns#',
			);
			$prefixes = apply_filters( 'crocal_eutf_opengraph_prefixes', $prefixes );
...
add_filter( 'language_attributes', 'crocal_eutf_opengraph_add_prefix' );

if ( !function_exists('crocal_eutf_print_meta_tags') ) {
	function crocal_eutf_print_meta_tags() {
		...
		if ( crocal_eutf_visibility( 'meta_viewport_responsive', '1' ) ) {
		...
		//Social Meta Tags
		if ( crocal_eutf_visibility( 'meta_opengraph' ) || crocal_eutf_visibility( 'meta_twitter' ) ) {
			...
			if ( is_singular() || is_front_page() || is_home() ) {
				...
				//Twitter Cards
				if ( crocal_eutf_visibility( 'meta_twitter' ) ) {
		...
		$meta_name_tags = apply_filters( 'crocal_eutf_meta_name_tags', $meta_name_tags );
		$meta_property_tags = apply_filters( 'crocal_eutf_meta_property_tags', $meta_property_tags );
...
add_action( 'wp_head', 'crocal_eutf_print_meta_tags', 5 );
