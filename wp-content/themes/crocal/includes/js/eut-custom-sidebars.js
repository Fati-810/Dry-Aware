	...
	$(document).on("click","#eut-add-custom-sidebar-item",function() {
		...
		if ( '' == $.trim(sidebarName) ) {
			...
			if ( alreadyExists ) {
				...
				var dataParams = {
					action:'crocal_eutf_get_custom_sidebar',
					sidebar_name: sidebarName,
					_eutf_nonce: crocal_eutf_custom_sidebar_texts.nonce_custom_sidebar
				};
				$.post( crocal_eutf_custom_sidebar_texts.ajaxurl, dataParams, function( sidebarHtml ) {
