jQuery(document).ready(function($) {
	...
	$(document).on("click",".eut-upload-replace-image",function() {
		...
		eutMediaImageReplaceFrame = wp.media.frames.eutMediaImageReplaceFrame = wp.media({
			...
			frame: 'select',
			multiple: false,
			title: crocal_eutf_upload_image_replace_texts.modal_title,
			library: {
				type: 'image'
			},
			button: {
				text:  crocal_eutf_upload_image_replace_texts.modal_button_title
			}
		...
		eutMediaImageReplaceFrame.on('select', function(){
			...
			var dataParams = {
				action:'crocal_eutf_get_replaced_image',
				attachment_id: ids.toString(),
				attachment_mode: eutMediaImageReplaceMode,
				field_name: eutMediaImageFieldName,
				_eutf_nonce: crocal_eutf_upload_image_replace_texts.nonce_replace
			};
			$.post( crocal_eutf_upload_image_replace_texts.ajaxurl, dataParams, function( mediaHtml ) {
