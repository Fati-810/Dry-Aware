    ...
    if ( ! class_exists( 'Redux_Helpers' ) ) {
        ...
        class Redux_Helpers {
            ...
            public static function compileSystemStatus( $json_output = false, $remote_checks = false ) {
				...
				//REDUX_MOD_TOM
                $sysinfo['redux_data_writeable'] = 'true';
                    ...
                    foreach ( $files as $file ) {
                        ...
                        if ( $theme_file ) {
                            ...
                            if ( $core_version && ( empty( $theme_version ) || version_compare( $theme_version, $core_version, '<' ) ) ) {
                                ...
                                $found_files[ $plugin_name ][] = sprintf( __( '<code>%s</code> version <strong>%s</strong> is out of date. The core version is %s', 'crocal' ), str_replace( WP_CONTENT_DIR . '/themes/', '', $theme_file ), $theme_version ? $theme_version : '-', $core_version );
                            } else {
