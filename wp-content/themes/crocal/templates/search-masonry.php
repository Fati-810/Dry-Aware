...
<?php
	...
	$search_page_mode = crocal_eutf_option( 'search_page_mode', 'masonry' );

	if ( 'yes' == $search_page_show_image ) {
		$search_image_mode = crocal_eutf_option( 'search_image_mode', 'landscape' );
		$search_masonry_image_mode = crocal_eutf_option( 'search_masonry_image_mode', 'medium' );
...
<article id="eut-search-<?php the_ID(); ?><?php echo uniqid('-'); ?>" <?php post_class( 'eut-blog-item eut-isotope-item' ); ?>>
	...
	<?php if ( 'yes' == $search_page_show_image && has_post_thumbnail() ) { ?>
		<div class="eut-media eut-image-hover clearfix">
			<a href="<?php echo esc_url( get_permalink() ); ?>"><?php crocal_eutf_the_post_thumbnail( $image_size, array( 'data-column-space' => 100 ) ); ?></a>
		</div>
	<?php } ?>
